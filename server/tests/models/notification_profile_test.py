...
@pytest.mark.django_db
def test_send_skipped(basic_event, group, monkeypatch):
    ...
    # Create a notification delivery method
    ndm = EmailNotificationDeliveryMethod.objects.create(name="Test NDM",
          created_by_group=group, enabled=False,
          email_to_addresses=["test@example.com"],          
    )
    ...
    assert notification.event == basic_event
    assert notification.notification_delivery_method == ndm
    assert notification.send_status == NotificationSendStatus.SKIPPED
    assert notification.send_result is None
    assert notification.attempted_at is not None
    assert notification.completed_at is None
    assert notification.exception_type is None
    assert notification.exception_message is None


@pytest.mark.django_db
def test_send_while_disabled(basic_event, group, monkeypatch):
    ...
    monkeypatch.setattr(EmailNotificationDeliveryMethod, 'send', patched_send,
          raising=False)
