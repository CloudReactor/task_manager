...
from processes.models import (
  PagerDutyProfile, RunEnvironment, UserGroupAccessLevel
)

from processes.serializers import PagerDutyProfileSerializer
...
def ensure_serialized_pagerduty_profile_valid(response_entity: Dict[str, Any],
    ...
    context = context_with_authenticated_request(user=user,
            group=pagerduty_profile.created_by_group,
    ...
    assert response_entity == PagerDutyProfileSerializer(pagerduty_profile,
...
def test_pagerduty_profile_list(
        ...
        pager_duty_profile_factory, api_client) -> None:
    user = user_factory()
    ...
    pagerduty_profiles = [
      pager_duty_profile_factory(name='Production', created_by_group=pdp_group,
              run_environment=production_run_environment),
      pager_duty_profile_factory(name='Scopeless', created_by_group=pdp_group,
              run_environment=None),
      pager_duty_profile_factory(name='Test', created_by_group=pdp_group,
    ...
    response = client.get('/api/v1/pagerduty_profiles/', params)
    ...
    if status_code == 200:
        ...
        for i in expected_indices:
            ...
            target_am = pagerduty_profiles[expected_indices[i]]
            ensure_serialized_pagerduty_profile_valid(response_entity,
                    pagerduty_profile=target_am, user=user,
...
def common_setup(is_authenticated: bool, group_access_level: Optional[int],
        ...
        user, group_factory, run_environment_factory, pager_duty_profile_factory, api_client) \
        -> Tuple[PagerDutyProfile, Optional[RunEnvironment], APIClient, str]:
    group = user.groups.first()
    ...
    pagerduty_profile = pager_duty_profile_factory(created_by_group=am_group,
    ...
    url = '/api/v1/pagerduty_profiles/'

    if uuid_send_type != SEND_ID_NONE:
        entity_uuid = uuid.uuid4()
        if uuid_send_type == SEND_ID_CORRECT:
            entity_uuid = pagerduty_profile.uuid
        elif uuid_send_type == SEND_ID_IN_WRONG_GROUP:
            another_group = group_factory()
            set_group_access_level(user=user, group=another_group,
                access_level=UserGroupAccessLevel.ACCESS_LEVEL_ADMIN)
            pagerduty_profile_in_other_group = pager_duty_profile_factory(
                    created_by_group=another_group)
            entity_uuid = pagerduty_profile_in_other_group.uuid

        url += quote(str(entity_uuid)) + '/'

    return (pagerduty_profile, api_key_run_environment, client, url)
...
def make_request_body(uuid_send_type: Optional[str],
    ...
    if uuid_send_type == SEND_ID_NOT_FOUND:
        ...
        request_data['uuid'] = str(pagerduty_profile.uuid)
...
def test_pagerduty_profile_fetch(
        is_authenticated: bool, group_access_level: Optional[int],
        ...
        pager_duty_profile_factory, api_client) -> None:
    user = user_factory()
    pagerduty_profile, api_key_run_environment, client, url = common_setup(
    ...
    if status_code == 200:
        assert group_access_level is not None
        ensure_serialized_pagerduty_profile_valid(response_entity=response.data,
...
def test_pagerduty_profile_create_access_control(
    ...
    old_count = PagerDutyProfile.objects.count()
    ...
    request_data = make_request_body(uuid_send_type=body_uuid_type,
            ...
            group_factory=group_factory,
            api_key_run_environment=api_key_run_environment,
            pagerduty_profile=pagerduty_profile,
            run_environment_factory=run_environment_factory)
    ...
    new_count = PagerDutyProfile.objects.count()
    ...
    if status_code == 201:
        ...
        assert group_access_level is not None
        ensure_serialized_pagerduty_profile_valid(response_entity=response_entity,
          pagerduty_profile=created_am, user=user,
          group_access_level=group_access_level,
          api_key_access_level=api_key_access_level,
          api_key_run_environment=api_key_run_environment)
    else:
...
def test_pagerduty_profile_update_access_control(
        is_authenticated: bool, group_access_level: Optional[int],
        api_key_access_level: Optional[int], api_key_scope_type: str,
    ...
    assert response.status_code == status_code
    assert PagerDutyProfile.objects.count() == old_count

    pagerduty_profile.refresh_from_db()
...
""", [
  ...
  # Support with JWT token fails with 403
  (True, UserGroupAccessLevel.ACCESS_LEVEL_SUPPORT,
   None, None,
   SEND_ID_CORRECT, True,
   403),
...
def test_pagerduty_profile_delete(
        is_authenticated: bool, group_access_level: Optional[int],
        api_key_access_level: Optional[int], api_key_scope_type: str,
        uuid_send_type: str, existing_has_run_environment: bool,
        status_code: int,
    ...
    exists = PagerDutyProfile.objects.filter(pk=pagerduty_profile.pk).exists()
